import { downloadCacheAsync, uploadCacheAsync } from '@expo/build-tools';
...
describe(GCSCacheManager, () => {
  ...
  async function saveAndRestoreCacheAsync(
    ...
    jest.mocked(downloadCacheAsync).mockImplementation(async () => {
      downloadDir = await fs.mkdtemp(path.join(os.tmpdir(), 'eas-cache-download-'));
    ...
    const uploadedKey = jest.mocked(uploadCacheAsync).mock.calls[0][0].key;
    expect(downloadCacheAsync).toHaveBeenCalledWith(expect.objectContaining({ key: uploadedKey }));
    expect(downloadDir).toBeDefined();
    await expect(fs.pathExists(downloadDir!)).resolves.toBe(false);
  }
  ...
  test('saves after a cache miss', async () => {
    const manager = new GCSCacheManager();
  ...
  test('does not save when restore fails', async () => {
    ...
    const mockCtx = createMockCtx({ paths: ['index.ts'] });
    await fs.outputFile(path.join(tmpDir, 'build', 'index.ts'), 'index.ts');
    jest.mocked(downloadCacheAsync).mockRejectedValue(new Error('download failed'));
