...
USAGE
  $ eas sim [-p android|ios] [--name <value>] [--tag <value>...] [--device <value>] [--build-id <value> |
    ...
    [--max-duration-minutes <value>] [--max-idle-time-minutes <value>] [--force] [--out-config-type env|dotenv] [--json]
    [--non-interactive]
...
FLAGS
  -p, --platform=<option>                Device platform
      ...
      --tag=<value>...                   Label used to group simulator sessions, for example one per app variant. Repeat
                                         for multiple tags. Stored lowercased.
