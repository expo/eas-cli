...
describe(WorkflowRunList, () => {
  ...
  test('list workflow runs with specific filename', async () => {
    ...
    jest.mocked(AppQuery.byIdWorkflowsAsync).mockResolvedValue([
      {
        id: 'build',
        name: 'build',
        fileName: 'build.yml',
    ...
    jest.mocked(WorkflowQuery.byIdRunsAsync).mockResolvedValue(getMockEmptyWorkflowRunsFragment());
    const cmd = mockTestCommand(WorkflowRunList, ['--workflow', 'build.yml'], ctx);
    await cmd.run();
    expect(WorkflowQuery.byIdRunsAsync).toHaveBeenCalledWith(
      ctx.loggedIn.graphqlClient,
      'build',
      10
    );
