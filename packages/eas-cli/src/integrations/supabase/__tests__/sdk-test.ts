...
describe('installSdkPackagesAsync', () => {
  ...
  it('returns installed on success', async () => {
    ...
    await expect(installSdkPackagesAsync('/project', false)).resolves.toEqual({
      status: 'installed',
    });
    expect(spawnAsync).toHaveBeenCalledWith(
      'npx',
      ['expo', 'install', ...SDK_PACKAGES],
  ...
  it('returns dynamic config guidance when install output includes it', async () => {
    ...
    await expect(installSdkPackagesAsync('/project', true)).resolves.toEqual({
      status: 'installed',
...
describe('addConfigPluginAsync', () => {
  ...
  it('skips when plugin is already present as a string or tuple', async () => {
    await expect(
      addConfigPluginAsync('/project', {
        ...exp,
        plugins: [[CONFIG_PLUGIN, {}], 'other'],
      })
    ).resolves.toBeNull();
    await expect(
      addConfigPluginAsync('/project', {
  ...
  it('returns null on successful modification', async () => {
    ...
    await expect(addConfigPluginAsync('/project', exp)).resolves.toBeNull();
  });
  ...
  it('returns warn message when modification warns', async () => {
    ...
    await expect(addConfigPluginAsync('/project', exp)).resolves.toContain('dynamic config');
  });
  ...
  it('returns fallback message for other modification results', async () => {
    ...
    await expect(addConfigPluginAsync('/project', exp)).resolves.toContain(
      JSON.stringify(CONFIG_PLUGIN)
    );
  });
});
...
describe('setupSdkAndConfigAsync', () => {
  ...
  it('adds install failure guidance', async () => {
    ...
    const steps = await setupSdkAndConfigAsync('/project', exp, true);
