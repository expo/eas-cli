...
export default class ProjectLink extends EasCommand {
  static override description = 'link a local project to an EAS project';
  static override aliases = ['link'];
  ...
  protected static async saveProjectIdAndLogSuccessAsync(
    projectDir: string,
    projectId: string
  ): Promise<void> {
  ...
  protected static async modifyExpoConfigAsync(
    projectDir: string,
  ...
  protected static async ensureOwnerSlugConsistencyAsync(
  ...
  protected static async setExplicitIDAsync(
    projectId: string,
    projectDir: string,
    { force, nonInteractive }: InitializeMethodOptions
  ): Promise<void> {
    ...
    if (!existingProjectId) {
      await ProjectLink.saveProjectIdAndLogSuccessAsync(projectDir, projectId);
      return;
    }
  ...
  protected static async initializeWithExplicitIDAsync(
    projectId: string,
    projectDir: string,
    { force, nonInteractive }: InitializeMethodOptions
  ): Promise<void> {
  ...
  protected static async initializeWithoutExplicitIDAsync(
    graphqlClient: ExpoGraphqlClient,
    actor: Actor,
    projectDir: string,
    { force, nonInteractive }: InitializeMethodOptions
    ...
    if (!accountName) {
      if (allAccounts.length === 1) {
        ...
        const choices = ProjectLink.getAccountChoices(
          actor,
    ...
    if (existingProjectIdOnServer) {
      ...
      await ProjectLink.saveProjectIdAndLogSuccessAsync(projectDir, existingProjectIdOnServer);
    ...
    await ProjectLink.saveProjectIdAndLogSuccessAsync(projectDir, createdProjectId);
    return createdProjectId;
  }

  protected static getAccountChoices(
    actor: Actor,
  ...
  async runAsync(): Promise<void> {
    ...
    } = await this.parse(ProjectLink);
    const {
      loggedIn: { actor, graphqlClient },
      projectDir,
    } = await this.getContextAsync(ProjectLink, { nonInteractive });

    let idForConsistency: string;
    if (idArgument) {
      await ProjectLink.initializeWithExplicitIDAsync(idArgument, projectDir, {
        force,
        nonInteractive,
      });
      idForConsistency = idArgument;
    } else {
      idForConsistency = await ProjectLink.initializeWithoutExplicitIDAsync(
        graphqlClient,
        actor,
        projectDir,
        {
    ...
    await ProjectLink.ensureOwnerSlugConsistencyAsync(graphqlClient, idForConsistency, projectDir, {
      force,
      nonInteractive,
    });
  }
