...
jest.mock('../../../commandUtils/context/ProjectIdContextField');
    ...
    describe('when it is already configured', () => {
      ...
      describe('interactive', () => {
        it('is no-op if already configured for id', async () => {
          ...
          expect(ProjectIdContextField['saveProjectIdToAppConfigAsync']).not.toHaveBeenCalled();
        });

        it('prompts to overwrite when different', async () => {
          jest.mocked(confirmAsync).mockResolvedValue(true);
          await new ProjectInit(['--id', '12345'], commandOptions).run();
          expect(ProjectIdContextField['saveProjectIdToAppConfigAsync']).toHaveBeenCalledWith(
            '/test-project',
            '12345'
          );
