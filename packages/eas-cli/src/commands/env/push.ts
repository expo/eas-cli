...
export default class EnvironmentValuePush extends EasCommand {
  ...
  async runAsync(): Promise<void> {
    ...
    const existingSharedVariables = existingVariables.filter(
    ...
    if (existingSharedVariables.length > 0) {
      const existingSharedVariablesNames = existingSharedVariables.map(variable => variable.name);
      Log.error(
        'Shared variables cannot be overwritten.\nPlease remove them from the env file or unlink them from the project:'
      );
      existingSharedVariablesNames.forEach(name => Log.error(`- ${name}`));
      return;
    }
    ...
    content
      ...
      .forEach(line => {
        ...
        if (name && value && name.match(/^\w+$/)) {
          variables[name] = { name, value, environment, sensitive: name.startsWith('EXPO_SECRET') };
        }
      });
    return variables;
  }
