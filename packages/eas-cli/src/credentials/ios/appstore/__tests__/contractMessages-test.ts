...
describe(formatContractMessage, () => {
  it(`formats expired message`, () => {
    expect(formatContractMessage(expiredMembership as any)).toBe('');
  });
  it(`formats payment message`, () => {
    expect(formatContractMessage(paymentReturned as any)).toBe('');
  });
  it(`formats license message`, () => {
    expect(formatContractMessage(licenseAgreementUpdated as any)).toBe('');
  });
  it(`formats paid license message`, () => {
    expect(formatContractMessage(paidLicenseAgreementUpdated as any)).toBe('');
  });
});
