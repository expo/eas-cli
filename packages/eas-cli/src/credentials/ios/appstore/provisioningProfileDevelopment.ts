...
async function findProfileByIdAsync(
  ...
  profiles = profiles.filter(
    profile => profile.attributes.profileType === ProfileType.IOS_APP_DEVELOPMENT
  );
  return profiles.find(profile => profile.id === profileId) ?? null;
}

async function manageDevelopmentProfilesAsync(
  context: RequestContext,
  {
    udids,
    bundleId,
  ...
  const newProfile = await Profile.createAsync(context, {
    ...
    name: `*[expo] ${bundleId} Development ${Date.now()}`,
...
export async function createOrReuseDevelopmentProvisioningProfileAsync(
  authCtx: AuthCtx,
  udids: string[],
  ...
  const spinner = ora(`Handling Apple development provisioning profiles`).start();
  try {
    const context = getRequestContext(authCtx);
    const { didUpdate, didCreate, profileName, ...developmentProvisioningProfile } =
      await manageDevelopmentProfilesAsync(context, {
        udids,
