...
export function buildObserveVersionsTable(results: AppVersionsResult[]): string {
  const allVersions: { platform: AppPlatform; version: AppObserveAppVersion }[] = [];
  for (const { platform, appVersions } of results) {
  ...
  const headers = ['App Version', 'Platform', 'First Seen', 'Events', 'Users', 'Builds', 'Updates'];

  const rows: string[][] = allVersions.map(({ platform, version }) => [
    version.appVersion,
