    ...
    Field(node) {
      ...
      requiredFields.forEach(field => {
        if (fieldAvailableOnType(def.type, field)) {
          ...
          if (!fieldWasRequested) {
            context.reportError(
              new _graphql.GraphQLError(`'${field}' field required on '${node.name.value}'`, [node])
            );
          }
        }
      });
  ...
  return {
    NamedType(node) {
      ...
      if (typeName[0] == typeName[0].toLowerCase()) {
        context.reportError(
          new _graphql.GraphQLError('All type names should start with a capital letter', [node])
        );
      }
    },
  };
