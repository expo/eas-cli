...
// Shell steps spawn in the project target directory, which the mock context does not create.
...
describe('StepsConfigParser local single-step functions', () => {
  describe('expansion', () => {
    ...
    it('rejects working_directory on a step that calls a single-step function', async () => {
      await expect(
        ...
        /"working_directory" is not supported on a step that calls a local function/
      );
    });
  ...
  describe('inside a composite function', () => {
    ...
    it('rejects working_directory on a nested call to a single-step function', async () => {
