...
export function createStartAndroidEmulatorBuildFunction(): BuildFunction {
  return new BuildFunction({
    ...
    fn: async ({ logger }, { inputs, outputs, env }) => {
      ...
      const { serialId, emulatorPromise } = await startAndroidEmulatorAsync({
        deviceName,
        ...
        adjustAnimationScale: shouldAdjustAnimationScale,
        logcatDirectory,
        env,
        logger,
      });
...
type AndroidEmulatorStartResult = Awaited<ReturnType<typeof AndroidEmulatorUtils.startAsync>>;

/**
 * Installs the system image if needed, then creates, starts, and waits for one
 * Android emulator, retrying with growing timeouts. Shared by the
 * `eas/start_android_emulator` step and the device run session runner.
 */
export async function startAndroidEmulatorAsync({
  deviceName,
  deviceIdentifier,
  systemImagePackage,
  lcdWidth,
  ...
  adjustAnimationScale: boolean;
  logcatDirectory: string;
  env: BuildStepEnv;
  logger: bunyan;
}): Promise<{
  serialId: AndroidEmulatorStartResult['serialId'];
  emulatorPromise: AndroidEmulatorStartResult['emulatorPromise'];
}> {
  ...
  let emulatorPromise: AndroidEmulatorStartResult['emulatorPromise'] | null = null;
  let serialId: AndroidEmulatorStartResult['serialId'] | null = null;
  await retryAsync(
    async attemptCount => {
      const timeoutMs = ANDROID_STARTUP_ATTEMPT_TIMEOUT_MS[attemptCount];
      const attempt = attemptCount + 1;
  ...
  return { serialId, emulatorPromise };
}

export async function assertAndroidEmulatorHostSupportAsync({
  env,
}: {
