import { BuildJob, ManagedArtifactType, Platform } from '@expo/eas-build-job';
...
async function resolveSourceMapPathAsync(ctx: BuildContext<BuildJob>): Promise<string> {
  if (ctx.job.platform === Platform.IOS) {
    const sourceMapPath = await prepareIosSourceMapPathAsync(ctx);
    if (!(await fs.pathExists(sourceMapPath))) {
      throw new Error(`The iOS source map was not generated at ${sourceMapPath}.`);
    }
    return sourceMapPath;
  }
  ...
  if (ctx.job.platform === Platform.ANDROID) {
    ...
    if (sourceMapPaths.length === 0) {
      throw new Error('The Android build did not generate a final composed source map.');
    }
  ...
  throw new Error('Source-map upload is not supported for this build platform.');
}
...
async function stripSourcesContentAsync(
  ...
  if (!isRecord(sourceMap) || sourceMap.version !== 3) {
    throw new Error(`Invalid source map at ${sourceMapPath}.`);
  }
