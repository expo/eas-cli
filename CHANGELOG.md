...
- [build-tools] Add the `eas/start_local_egress` step for iOS simulator sessions with local egress: the device host routes the simulator's traffic through a reverse tunnel to the EAS CLI machine, so third parties see that machine's IP. ([#4364](https://github.com/expo/eas-cli/pull/4364) by [@brentvatne](https://github.com/brentvatne))
