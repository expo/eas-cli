...
- [build-tools] Start serve-sim with `--network-capture` and `--enable networkCapture`, and with `--network-capture-field` for each requested field, when a simulator session requests network capture. ([#4307](https://github.com/expo/eas-cli/pull/4307) by [@gwdp](https://github.com/gwdp))
