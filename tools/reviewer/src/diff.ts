import type { DiffEntry } from './schema.ts';
...
export function parseUnifiedDiff(diffText: string): DiffEntry[] {
  ...
  const flush = (): void => {
    ...
    const entry = patchToEntry(patch);
    if (entry) {
      entries.push(entry);
    }
    current = null;
...
function patchToEntry(patch: string): DiffEntry | null {
  ...
  // For deletions the new path is /dev/null; keep the old path so the entry is
  // still addressable, but mark it deleted.
...
function stripDiffPathPrefix(raw: string): string {
  ...
  // Strip the leading a/ or b/ that git prepends.
